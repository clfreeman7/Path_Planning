...
addpath('../');
addpath('data/visualtracking/MTA4 32 NS');
...
for i = 1:5
    % Define robot.
    exp_NS_32(i).params.robot_name = 'orange';
    exp_NS_32(i).params.substrate = 'black mat';
    
    exp_NS_32(i).params.n_markers = 4;
    exp_NS_32(i).params.pixel_length = 1/8.6343;     % cm per pixel
    
    % Extract and store first frame information.
    exp_NS_32(i).params.frame_1 = frame_start_list(i);

        % Look at image to determine marker order.
    exp_NS_32(i).params.marker_order = marker_order_list(i, :);
    % Extract and store raw data from each trial.
    filename = ['tracked_data_', num2str(i),'.mat'];
    exp_NS_32(i).raw_data = load(filename).tracking_data;  
end
for iTrial = 1:5
    % Find initial rotation matrix for each trial to have consistent fixed frame.
        markers_x(iTrial, :) = exp_NS_32(iTrial).raw_data(1, 1:3:exp_NS_32(i).params.n_markers*3-2);
    markers_y(iTrial, :) = exp_NS_32(iTrial).raw_data(1, 2:3:exp_NS_32(i).params.n_markers*3-1);
    centroid(:, :, iTrial) = mean([markers_x(iTrial, :); markers_y(iTrial, :)], 2);
    if iTrial == 1
        reference_markers = [markers_x(iTrial, exp_NS_32(iTrial).params.marker_order);
                             markers_y(iTrial, exp_NS_32(iTrial).params.marker_order);]...
                             - centroid(:, :, iTrial);
    else
        shifted_markers = [markers_x(iTrial, exp_NS_32(iTrial).params.marker_order);
                           markers_y(iTrial, exp_NS_32(iTrial).params.marker_order)]...
                           - centroid(:, :, iTrial);
        % Find orientation of subsequent trials w.r.t. first trial GCS.
         ...
         exp_NS_32(iTrial).params.R_1 = [regParams.R zeros(2,1); 0 0 1];
    end
end
...
for i = 1:5
    all_trials(i) = offlineanalysis.PrimitivesTest(exp_NS_32(i).raw_data, exp_NS_32(i).params, state_order_NS(i,:));
    nexttile;
    all_trials(i).plot;
...
save data/NS_32_motion_primitives.mat motion_primitive_data
...
figure(3)
t_NS_32 = plot_all_deviations(motion_primitive_data);
title(t_NS_32, 'Motion primitive variation for no sheath orange robot on black mat')
...
if show_markers
    ...
    markers_x(i, :) = exp_NS_32(i).raw_data(1, 1:3:exp_NS_32(i).params.n_markers*3-2);
    markers_y(i, :) = exp_NS_32(i).raw_data(1, 2:3:exp_NS_32(i).params.n_markers*3-1);
%     if iTrial ~=5
    for m = 1:exp_NS_32(i).params.n_markers
