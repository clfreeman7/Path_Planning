...
frame_start_list = [291,114,96,111,108,99,102,105,119,117];
marker_order = repmat([2 4 6 5 3 1], 10,1);
marker_order([1,2,3,6,7,10],:) = repmat([3 5 6 4 2 1],6,1);
...
for i = 1:10
    ...
    exp(i).params.substrate = 'whiteboard';
    ...
    filename = ['data/visualtracking/MTA3/Whiteboard/Euler_', num2str(i), '_L.mat'];
    if ~isfile(filename)
        filename = ['data/visualtracking/MTA3/Whiteboard/Euler_', num2str(i), '_L_stitched.mat'];
    end
...
for i = 1:10
    ...
    file_name = ['data/visualtracking/MTA3/Whiteboard/', pic_name, '_L.jpg'];
...
title(t, 'Motion primitive exploration (Euler tours) for orange robot on whiteboard','FontSize',24)
...
params_3.substrate= 'Whiteboard';
params_3.n_unique_states = 8;
params_3.alpha_var = [1 1 1]';
params_3.MAX_ROTATION = deg2rad(1);
params_3.MAX_TRANSLATION = .2;
params_3.goal = 1;
params_3.alpha_len = 1.5;
params_3.n_variations = 100;
...
title(t,'Predicted vs. Actual Motion for each MTA3 Motion Primitive in an Euler Tour on Black Mat', 'FontSize', 24)
