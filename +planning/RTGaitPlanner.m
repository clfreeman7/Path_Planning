% ======================= RTGaitPlanner =======================
%
%  This gait-based trajectory planner is a simple extension of an 
%  underlying motion primitive-based controlled trajectory planner,
%  pathGen.RTGreedyPlanner. It's purpose is to provide a (more) convenient
%  user-interface for gait-based controlled trajectory planning.
% 
%  RTGreedyPlanner uses a grid world representation and takes a 
%  'greedy' search-based approach, finding a sequence of 
%  rotation-translation motion primitives that, collectively, 
%  travel from start to goal. RTGaitPlanner sub-classes RTGreedyPlanner
%  and somewhat trivially bridges the gap between 'motion primitive' 
%  planning and 'gait' planning. While RTGreedyPlanner allows more liberal 
%  configuration of the planning parameters, RTGaitPlanner constrains some 
%  aspects of the search for the context of gait-based planning;
%  it additionally serves to transcribe gait-based trajectory data types 
%  to/from motion primtive-based ones (i.e. user interface data structures 
%  to/from those used by the underlying superclass).
%
%  planning.RTGaitPlanner()
%
%  Dependencies:
...
%
%  ====================== RTGaitPlanner ========================
classdef RTGaitPlanner < pathGen.RTGreedyPlanner

  properties (Access = protected)
    rotation_gaits;        % gaitdef.Gait instance modeling rotationally-dominant MSoRo gait
    translation_gaits;     % gaitdef.Gait instance modeling translationally-dominant MSoRo gait
  ...
  methods (Access = public)
    ...
    % Constructor
    function this = RTGaitPlanner( params )
      assert( isfield(params, 'gridS'), '[RTGaitPlanner::RTGaitPlanner()] Missing gridS parameter!');
    ...
    %
    % Input(s):
    %   a_rot_gait:             Array of rotational gait models for trajectory planning
    %                           (array of class instances: gaitdef.Gait) 
    %   a_trans_gait:           Array of translational gait models for trajectory planning
    %                           (array class instances: gaitdef.Gait) 
    % 
    function setGaits( this, a_rot_gaits, a_trans_gaits )
      assert ( isa(a_rot_gaits, 'gaitdef.Gait'), ...
                  '[RTGaitPlanner::setGaits()] Added gait must be of type gaitdef.Gait.');

      % Add rotational gait model(s)
      this.rotation_gaits = a_rot_gaits;

      for ii = 1:length(a_rot_gaits)
        assert ( isa(a_rot_gaits(ii), 'gaitdef.Gait'), ...
                    '[RTGaitPlanner::setGaits()] Added gait must be of type gaitdef.Gait.');

        rot_gait_period = a_rot_gaits(ii).len_gait*a_rot_gaits(ii).transition_time;
  
        % Transcribe rotate gait data as motion primitive
        rot_mp_params.name = a_rot_gaits(ii).gait_name;
        rot_mp_params.body_vel_twist = a_rot_gaits(ii).Twist;
      ...
      % Add translational gait model(s) to planner
      this.translation_gaits = a_trans_gaits;

      for ii = 1:length(a_trans_gaits)
        assert ( isa(a_trans_gaits(ii), 'gaitdef.Gait'), ...
                  '[RTGaitPlanner::setGaits()] Added gait must be of type gaitdef.Gait.');

        trans_speed = norm(a_trans_gaits(ii).Twist(1:2));
...
%         trans_max_moves = floor(3/2*pi/a_rot_gaits.Twist(3));   % [TODO] this is original; vaguely remember intentionally coding rot-trans dependency - don't remember why
        ...
        trans_mp_params.body_vel_twist = a_trans_gaits(ii).Twist;   % [ cm/s, cm/s, rad/s ]
    ...
    function [ gait_trajectory_plan ] = planTrajectory( this, a_start_pose, a_goal_position )
      ...
      planner_primed = true;
      if ( ~this.gaits_set )
        warning('[RTGaitPlanner::planTrajectory()] Gaits not setup.');
        planner_primed = false;
      end
      if ( ~this.scenario_set )
        warning('[RTGaitPlanner::planTrajectory()] Scenario not set.');
        planner_primed = false;
      end
      if ( ~planner_primed )
        error('[RTGaitPlanner::planTrajectory()] Planner not properly setup.');
      end
    ...
    function [ gait_trajectory ] = mp2GaitTrajectory( this, a_mp_trajectory )
      ...
      for ii = 1:length(mp_plan_mps)
        ...
        if ( mp_plan_mp_types(ii) == 'T' )  % translate gait
          trans_gait_period = -1;
          for jj = 1:length(this.translation_gaits)
            if (this.translation_gaits(jj).gait_name == mp_plan_mps(ii).name)
              trans_gait_period = this.translation_gaits(jj).len_gait*this.translation_gaits(jj).transition_time;
            end
          end
          if ( trans_gait_period <= 0 )
            warning('RTGaitPlanner::mp2GaitTrajectory(): No matching translational gait found for gait name: %s.', mp_plan_mps(ii).name);
          end
        ...
        elseif ( mp_plan_mp_types(ii) == 'R' ) % rotate gait
          rot_gait_period = -1;
          for jj = 1:length(this.rotation_gaits)
            if (this.rotation_gaits(jj).gait_name == mp_plan_mps(ii).name)
              rot_gait_period = this.rotation_gaits(jj).len_gait*this.rotation_gaits(jj).transition_time;
            end
          end
          if ( rot_gait_period <= 0 )
            warning('RTGaitPlanner::mp2GaitTrajectory(): No matching rotational gait found for gait name: %s.', mp_plan_mps(ii).name);
          end
          ...
          error('[RTGaitPlanner::mp2GaitTrajectory()] Encountered invalid mp_type value generated MP controlled trajectory plan: %s.', mp_plan_mp_types(ii));
        end
      end
...
%       end
      ...
      for ii = 1:length(traj_gait_types)
        ...
        elseif ( traj_gait_types(ii) == gaitdef.GaitType.TRANSLATE )
          ...
          error('[RTGaitPlanner::plotTrajectory()] Invalid gait type encountered in trajectory plan: %s.', traj_gait_types(ii));
        end
      end
      ...
      for ii = 1:length(traj_gait_names)
        for jj = 1:length(gait_lib)
          ...
          if ( jj == length(gait_lib) )
            error('[RTGaitPlanner::plotTrajectory()] Gait name planned in trajectory; not found in gait library: %s', traj_gait_names(ii));
          end
        end
      end
      ...
      for ii = 1:length(traj_gait_types)          
        ...
        for jj = 1:num_samples
          g_twist = SE2.exp(gait_twist', gait_dur/num_samples);
          g_next = g_0*g_twist;
